...
- 删除图表、卡片、任务、旧进度组件、通知、日志、状态栏、按钮和 Artifact UI。
- 过程反馈使用独立、临时的 SSE `progress` 信封，不重新引入组件生命周期。
...
5. V3 SSE 每帧包含一个 `progress` 或 `component` 及会话、请求、时间戳信息。
6. Progress 只包含受控业务阶段和文案，不进入历史或 Polling。
7. 业务错误由 Agent 生成安全文本；传输错误使用独立的强类型错误信封。
8. 客户端仅在 SSE 尚未收到有效载荷时降级 Polling，progress 也视为有效载荷。
9. 客户端只维护一条临时过程状态，发送期间禁用输入。
...
- XPD 查询过程可见且不暴露 SQL、工具名、参数或内部错误。
