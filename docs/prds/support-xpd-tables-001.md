...
5. 每个涉及 XPD 数据查询或分析的用户回合必须先调用无参数 `search_xpd_schema`，得到当次完整 Schema 证据；同回合未搜索时，`run_xpd_sql` 必须拒绝执行。纯寒暄和身份问答不调用 Schema 或 SQL 工具。
...
10. 系统提示词应将面向用户的产品身份定义为“XPD 数据查询分析助手”，并对寒暄和身份问句给出明确回答口径。这些普通用户消息仍进入模型，不增加确定性拦截或响应改写。
