...
5. 每个涉及 XPD 数据查询或分析的用户回合必须先调用无参数 `search_xpd_schema`，得到当次完整 Schema 证据；同回合未搜索时，`run_xpd_sql` 必须拒绝执行。纯寒暄、身份问答和无需 XPD 数据的安全通用任务不调用 Schema 或 SQL 工具。
...
10. 系统提示词应将面向用户的产品身份定义为“XPD 数据查询分析助手”，并对寒暄和身份问句给出明确回答口径。助手仍应直接完成模型已有知识范围内的安全常识、数学、翻译、写作和编程任务，不得仅因请求与 XPD 无关而拒答；这些普通用户消息继续进入模型，不增加确定性分类器或响应改写。
